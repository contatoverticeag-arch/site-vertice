...
function curatedTrends(p) {
  if (p.agro && p.pipeline) {
    return [
      { title: "Uso de dados de interação para priorizar contas e temas comerciais no agro", source: "curadoria Vértice", url: "" },
      { title: "Demonstrações práticas e experiências de campo como ponte entre tecnologia e confiança", source: "curadoria Vértice", url: "" }
    ];
  }
  if (p.retention) {
    return [
      { title: "Customer Advisory Boards e fóruns de escuta ganham força como ferramenta de retenção B2B", source: "curadoria Vértice", url: "" },
      { title: "Régua pós-evento orientada por sinais de conta ajuda a antecipar risco e expansão", source: "curadoria Vértice", url: "" }
    ];
  }
  if (p.launch) {
    return [
      { title: "Lançamentos B2B performam melhor quando combinam demonstração, prova e jornada de adoção", source: "curadoria Vértice", url: "" },
      { title: "Experiências interativas ajudam o cliente a testar valor antes da conversa comercial", source: "curadoria Vértice", url: "" }
    ];
  }
  return [
    { title: "Conteúdo técnico precisa virar ferramenta de decisão, não apenas palestra", source: "curadoria Vértice", url: "" },
    { title: "Comunidades e trilhas pós-evento sustentam autoridade além do encontro presencial", source: "curadoria Vértice", url: "" }
  ];
}

function appliedOpportunity(p, trendItems) {
  const signal = trendItems[0]?.title || "curadoria estratégica da Vértice";
  if (p.agro && p.pipeline) {
    return {
...
Sinais externos ou curadoria estratégica:
